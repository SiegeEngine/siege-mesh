...
use siege_mesh::{Mesh, Point3, StandardVertex, VertexType};

pub fn main() {
    let cube_mesh = define_mesh();
    cube_mesh
        .save(VertexType::Standard, "cube_standard.mesh")
        .unwrap();
    println!("Saved.");
}
...
fn define_mesh() -> Mesh<StandardVertex> {
    ...
    mesh.strings = vec![
        ...
        (
            "albedo".to_owned(),
            "cube_standard_albedomap.bc7.dds.zst".to_owned(),
        ),
        (
            "normal".to_owned(),
            "cube_standard_normalmap.bc7.dds.zst".to_owned(),
        ),
    ];
